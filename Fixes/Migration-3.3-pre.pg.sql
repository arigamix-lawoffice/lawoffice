-- Скрипт требуется выполнить перед обновлением схемы на 3.3
...
BEGIN
  IF EXISTS (
	...
	WHERE "table_name" = 'WfSatelliteTaskHistory' and "column_name" = 'KindID') THEN
	
	CREATE TABLE "temp_Kinds" (
		"RowID" uuid NOT NULL,
		"KindID" uuid NOT NULL,
		"KindCaption" text NOT NULL);
	
	INSERT INTO "temp_Kinds"
	SELECT "RowID", "KindID", "KindCaption"
	FROM "WfSatelliteTaskHistory"
...
ALTER SEQUENCE IF EXISTS "WorkflowEngineLogs_Order_seq" RENAME TO "seq_WorkflowEngineLogs_Order";
GO

CREATE OR REPLACE FUNCTION delete_old_index_if_exists()
RETURNS void AS
$$
BEGIN
	IF EXISTS (
		SELECT null FROM pg_indexes WHERE indexname = 'ndx_WorkflowEngineLogs_ProcessRowIDOrder'
	) THEN
	    -- Добавляем в Definition таблицы WorkflowEngineLogs колонку Order и индекс ndx_WorkflowEngineLogs_ProcessRowIDOrder, которые раньше генерировались миграцией.

		UPDATE "Tables"
		SET "Definition" = '<SchemeTable Partition="dd8eeaba-9042-4fb5-9e8e-f7544463464f" ID="f3fa0390-6444-4df6-8be5-cbad1fdd153e" Name="WorkflowEngineLogs" Group="WorkflowEngine" InstanceType="Cards" ContentType="Collections">
			<Description>Секция с логами процесса</Description>
			<SchemeComplexColumn IsSystem="true" IsPermanent="true" IsSealed="true" ID="f3fa0390-6444-00f6-2000-0bad1fdd153e" Name="ID" Type="Reference(Typified) Not Null" ReferencedTable="1074eadd-21d7-4925-98c8-40d1e5f0ca0e">
		...
		WHERE "ID" = 'f3fa0390-6444-4df6-8be5-cbad1fdd153e';
	
		DROP INDEX IF EXISTS "ndx_WorkflowEngineLogs_ProcessRowID";
	END IF;
END;
$$
LANGUAGE 'plpgsql';
...
-- Удаляем сгенерированный индекс
SELECT delete_old_index_if_exists();
GO

DROP FUNCTION delete_old_index_if_exists();
GO
