...
IF EXISTS (
	SELECT 1
	FROM [sys].[columns] 
	WHERE [object_id] = OBJECT_ID(N'dbo.WfSatelliteTaskHistory') AND [name] = N'KindID')
BEGIN
	EXECUTE sp_executesql N'
		SELECT [RowID], [KindID], [KindCaption]
		INTO [temp_Kinds]
		FROM [WfSatelliteTaskHistory] WITH (NOLOCK)
...
SET @Definition = N'<?xml version="1.0" encoding="utf-16"?>
...
IF EXISTS (SELECT * FROM [sys].[indexes] WHERE name = 'ndx_WorkflowEngineLogs_ProcessRowIDOrder')
	AND EXISTS (SELECT TOP 1 1 FROM [sys].[indexes] WHERE name = N'ndx_WorkflowEngineLogs_ProcessRowID')
BEGIN
	UPDATE [Tables]
	SET [Definition] = @Definition
	WHERE ID = 'f3fa0390-6444-4df6-8be5-cbad1fdd153e'

	DROP INDEX [ndx_WorkflowEngineLogs_ProcessRowID] ON [WorkflowEngineLogs]
END
GO
